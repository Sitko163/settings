...
class FlightAdmin(admin.ModelAdmin):
    ...
    def import_xlsx_view(self, request):
        ...
        if not xlsx_files:
            self.message_user(request, _("Пожалуйста, выберите хотя бы один .xlsx или .xlsm файл."), level=messages.ERROR)
            return HttpResponseRedirect("../")

        # Проверяем, есть ли незавершенные импорты
        # Если есть - не очищаем базу, продолжаем импорт
        # Если нет - очищаем базу перед импортом
        has_unfinished_imports = ImportProgress.objects.filter(is_completed=False).exists()
        
        if not has_unfinished_imports:
            # Очищаем базу вылетов перед импортом (справочники не трогаем)
        ...
        for xlsx_file in xlsx_files:
            if not (xlsx_file.name.endswith('.xlsx') or xlsx_file.name.endswith('.xlsm')):
                self.message_user(request, _(f"Файл '{xlsx_file.name}' не является .xlsx или .xlsm файлом."),
                                  level=messages.ERROR)
                return HttpResponseRedirect("../")
            ...
            for xlsx_file in xlsx_files:
                try:
                    logger.info(f"Начинаем импорт из файла: {xlsx_file.name}, размер: {xlsx_file.size} байт")
                    self.message_user(request, _(f"Начинаем импорт из файла: {xlsx_file.name}"), level=messages.INFO)

                    # Вычисляем hash файла для идентификации
                    import hashlib
                    xlsx_file.seek(0)  # Сбрасываем позицию файла
                    file_content = xlsx_file.read()
                    file_hash = hashlib.md5(file_content).hexdigest()
                    xlsx_file.seek(0)  # Возвращаем позицию для чтения
                    
                    # Проверяем, есть ли сохраненный прогресс для этого файла
                    ...
                    start_row = 5  # По умолчанию начинаем с строки 5
                    if import_progress and not import_progress.is_completed:
                        start_row = import_progress.last_processed_row + 1
                        self.message_user(request,
                                          _(f"📌 Найден сохраненный прогресс! Продолжаем импорт с строки {start_row} (было обработано {import_progress.last_processed_row} из {import_progress.total_rows})"),
                                          level=messages.INFO)
                        logger.info(f"Продолжаем импорт файла '{xlsx_file.name}' с строки {start_row} (hash: {file_hash})")
                    elif import_progress and import_progress.is_completed:
                        self.message_user(request,
                                          _(f"✓ Файл '{xlsx_file.name}' уже полностью импортирован. Пропускаем."),
                                          level=messages.INFO)
                        logger.info(f"Файл '{xlsx_file.name}' уже полностью импортирован, пропускаем")
                        continue
                    else:
                        ...
                        import_progress = ImportProgress.objects.create(
                            ...
                            total_rows=0,  # Будет обновлено после определения
                            total_created=0,
                            is_completed=False
                        )
                        logger.info(f"Создана новая запись прогресса для файла '{xlsx_file.name}' (hash: {file_hash})")

                    import openpyxl
                    # НЕ используем read_only режим, так как он не позволяет читать ячейки в произвольном порядке
                    # Используем обычный режим для возможности чтения ячеек через ws.cell()
                    # Это потребует больше памяти, но обеспечит стабильную работу
                    try:
                        if xlsx_file.name.endswith('.xlsm'):
                            wb = openpyxl.load_workbook(xlsx_file, data_only=True, keep_vba=True)
                        else:
                            wb = openpyxl.load_workbook(xlsx_file, data_only=True)
                        logger.info(f"Файл '{xlsx_file.name}' открыт в обычном режиме (для стабильной работы с большими файлами)")
                    except Exception as e:
                        # Если keep_vba не поддерживается, пробуем без него
                        logger.warning(f"Ошибка при открытии файла '{xlsx_file.name}' с keep_vba: {e}, пробуем без keep_vba")
                        wb = openpyxl.load_workbook(xlsx_file, data_only=True)
                    
                    # Ищем лист "СВОДНАЯ"
                    sheet_name = None
                    for sheet in wb.sheetnames:
                        if sheet.upper() == 'SVODNAYA' or 'СВОДНАЯ' in sheet.upper():
                            sheet_name = sheet
                            break
                    
                    if sheet_name is None:
                        self.message_user(request,
                                          _(f"Лист 'СВОДНАЯ' не найден в файле '{xlsx_file.name}'."),
                                          level=messages.ERROR)
                        continue
                    ...
                    for row_idx in range(data_start_row, data_end_row + 1):
                        ...
                        try:
                            ...
                            # Позывной оператора из колонки V
                                ...
                                try:
                                    with transaction.atomic():
                                        ...
                                        if flights_to_create:
                                            ...
                                            # Преобразуем координаты в фоновом режиме для созданных полетов
                                            # Обрабатываем все полеты с координатами, которые еще не преобразованы
                                            import threading
                                            def convert_coordinates_background():
                                                """Фоновая функция для преобразования координат"""
                                                try:
                                                    from flights.models import Flight
                                                    # Находим все полеты с координатами, которые еще не преобразованы
                                                    flights_to_convert = Flight.objects.filter(
                                                        ...
                                                        lat_wgs84__isnull=True
                                                    )[:100]  # Обрабатываем по 100 за раз
                                                    
                                                    converted_count = 0
                                                    for flight in flights_to_convert:
                                                        try:
                                                            # Преобразуем координаты и сохраняем в БД
                                                            flight.get_coordinates_info_cached()
                                                            converted_count += 1
                                                        except Exception as coord_error:
                                                            logger.warning(f"Ошибка преобразования координат для полета {flight.id}: {coord_error}")
                                                    
                                                    if converted_count > 0:
                                                        logger.info(f"Преобразовано координат в фоновом режиме: {converted_count} полетов")
                                                except Exception as bg_error:
                                                    logger.error(f"Ошибка в фоновом преобразовании координат: {bg_error}", exc_info=True)
                                            
                                            # Запускаем в фоновом потоке
                                            thread = threading.Thread(target=convert_coordinates_background, daemon=True)
                                            thread.start()
                                            logger.debug(f"Запущена фоновая задача преобразования координат")
                            ...
                            error_msg = f"Ошибка обработки строки {row_idx} в файле '{xlsx_file.name}': {str(e)}"
                                ...
                                if flights_to_create:
                                    ...
                                    if total_created_in_final != len(flights_to_create):
                                        ...
                                        self.message_user(request,
                                                          ...
                                                          level=messages.WARNING)
                                    # Преобразуем координаты в фоновом режиме для финального батча
                                    import threading
                                    def convert_coordinates_background_final():
                                        """Фоновая функция для преобразования координат финального батча"""
                                        try:
                                            from flights.models import Flight
                                            # Находим все полеты с координатами, которые еще не преобразованы
                                            flights_to_convert = Flight.objects.filter(
                                                ...
                                                lat_wgs84__isnull=True
                                            )[:200]  # Обрабатываем по 200 за раз для финального батча
                                            ...
                                            if converted_count > 0:
                                                logger.info(f"Преобразовано координат в фоновом режиме (финальный батч): {converted_count} полетов")
                                        except Exception as bg_error:
                                            logger.error(f"Ошибка в фоновом преобразовании координат (финальный батч): {bg_error}", exc_info=True)
                                    
                                    # Запускаем в фоновом потоке
                                    thread = threading.Thread(target=convert_coordinates_background_final, daemon=True)
                                    thread.start()
                                    logger.debug(f"Запущена фоновая задача преобразования координат для финального батча")
                    ...
                    if error_messages:
                        ...
                        if len(error_messages) > 5:
                            self.message_user(request,
                                              f"... и ещё {len(error_messages) - 5} ошибок в файле '{xlsx_file.name}'.",
                                              level=messages.WARNING)
                    ...
                    if import_progress:
                        try:
                            ...
                            if is_completed:
                                logger.info(f"Импорт файла '{xlsx_file.name}' завершен полностью (строка {final_row}, создано {created_count})")
                                ...
                                logger.info(f"Импорт файла '{xlsx_file.name}' приостановлен (строка {final_row}, создано {created_count}). Можно продолжить при следующей загрузке.")
                                self.message_user(request,
                    ...
                    import gc
                    gc.collect()
                    logger.info(f"Память очищена после обработки файла '{xlsx_file.name}'")

                except Exception as e:
                    import traceback
                    error_traceback = traceback.format_exc()
                    logger.error(f"Критическая ошибка при импорте из файла '{xlsx_file.name}': {str(e)}\n{error_traceback}")
                    ...
                    self.message_user(request,
                                      _(f"Критическая ошибка при импорте из файла '{xlsx_file.name}': {str(e)}"),
                                      level=messages.ERROR)
